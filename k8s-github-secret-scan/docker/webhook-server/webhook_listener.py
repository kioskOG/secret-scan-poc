...
try:
    producer = KafkaProducer(
        bootstrap_servers=['kafka.secret-scan.svc.cluster.local:9092'],  # Or use your Kafka broker address
    ...
    try:
        ...
        try:
            result = subprocess.run(
                ["/usr/local/bin/trufflehog", "filesystem", "--json", repo_dir],  # IMPORTANT: Scan the repo_dir
